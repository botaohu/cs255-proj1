...
\project{CS255: Introduction to Cryptography}{Winter 2013}{1}{Botao Hu, Borui Wang (botaohu, borui@stanford.edu)}{\today}{Encrypted Facebook groups}
...
In addition, to prevent to leak the group name information, we encrypt and decrypt the whole JSON string of the hash table of all group keys. For integrity, we use the same MAC technology with a seperate key in section \ref{sec:verify} to ensure the integrity of the data.
\begin{figure}[h!]
\centering
...
For integrity, we use the MAC with a seperate key to ensure the integrity of the data.
